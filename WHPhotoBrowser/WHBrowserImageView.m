...
#import "WHBrowserImageView.h"
#import "UIImageView+WebCache.h"
#import "WHPhotoBrowserConfig.h"
#import "Masonry.h"
//#define WS(weakSelf)  __weak __typeof(&*self)weakSelf = self;


//static const CGFloat kBrowserImageViewWidth = 320;
@interface WHBrowserImageView() <UIGestureRecognizerDelegate>

@end

@implementation WHBrowserImageView
{
    __weak WHWaitingView *_waitingView;
    BOOL _didCheckSize;
...
//            scroll.backgroundColor = WHPhotoBrowserBackgrounColor;
//            _scroll = scroll;
...
////            make.width.mas_equalTo(ws.bounds.size.width);
////            make.height.mas_equalTo(ws.bounds.size.height);
...
////        [_scrollImageView mas_makeConstraints:^(MASConstraintMaker *make) {
////            make.center.mas_equalTo(ws.center);
////            
////            make.width.mas_equalTo(ws.bounds.size.width);
////            make.height.mas_equalTo(ws.bounds.size.height);
...
//    WS(ws);
    ...
    WHWaitingView *waiting = [[WHWaitingView alloc] init];
//    waiting.bounds = CGRectMake(0, 0, 100, 100);
    waiting.mode = WHWaitingViewProgressMode;
    _waitingView = waiting;
    [self addSubview:waiting];
    [waiting mas_makeConstraints:^(MASConstraintMaker *make) {
        make.width.mas_equalTo(100.0);
        make.height.mas_equalTo(100.0);
        make.center.mas_equalTo(CGPointMake([UIScreen mainScreen].bounds.origin.x * 0.5, [UIScreen mainScreen].bounds.origin.y * 0.5));
    }];
    
    
    __weak WHBrowserImageView *imageViewWeak = self;
...
- (void)prepareForImageViewScaling
{
    if (!_zoomingScroolView) {
        ...
        _zoomingScroolView.backgroundColor = WHPhotoBrowserBackgrounColor;
